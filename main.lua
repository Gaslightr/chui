...
local NilInstance = Instance.new("Folder")
...
local function Vector2FromScale(X, Y)
    return Vector2_new(Camera.ViewportSize.X * X, Camera.ViewportSize.Y * Y)
end

local function Vector2FromScaleWithRatio(X, Y, DominantAxis, Ratio)
    local ScaledVector2 = Vector2FromScale(X, Y)
    X, Y = Camera.ViewportSize.X * X, Camera.ViewportSize.Y * Y
    ScaledVector2 = Vector2_new(DominantAxis == "X" and Y * Ratio or X, DominantAxis == "Y" and X * Ratio or Y)
...
function Library:CreateWindow(Title, X, Y, TitleHeight)
    ...
    function Window:Init()
        ...
        UIS.InputChanged:Connect(function(_)
            if _.UserInputType == Enum.UserInputType.MouseMovement then
                Window.MousePosition += Vector2_new(_.Delta.X, _.Delta.Y)
            end
        end)
    end
    function Window:CreateCategory(name)
        ...
        function Category:CreateSlider(name, minimum, maximum, state, callback)
            ...
            function Slider.Slid(Amount)    
                Amount = math.floor((Slider.MaxValue*(Amount/Slider.Hitbox.Size.X))+.5)
        ...
        function Category:CreateColorPicker(Items)
            local ColorPicker = {
                Type = "ColorPicker",
                SelectedItem = "",
                HueDragged = false,
            ...
            function ColorPicker:ForceColor(c)
                if ColorPicker.Items[ColorPicker.SelectedItem] ~= nil then
                    local h, s, v = c:ToHSV()
                    ColorPicker.Picker.Color = Color3.fromHSV(h, 1, 1)
    ...
    Connections.InputBegan = UIS.InputBegan:Connect(function(_)
        if _.UserInputType == Enum.UserInputType.MouseButton1 and IsVector2WithinDrawing(Window.MousePosition, Window.TitleBar) and Window.Visible then
            Window.BeingDragged = true
            Window.LastMouseClickOnTitle = Window.MousePosition
        end
        if _.UserInputType == Enum.UserInputType.MouseButton1 then
            for i, t in next, Window.Categories do
                if Window.Visible and IsVector2WithinDrawing(Window.MousePosition, t.Toggle.Hitbox) then
                ...
                for i, o in next, Window.CurrentCategory.Objects do
                    if o.Type == "ComboBox" and o.Open then
                        ...
                        for i, b in next, o.Buttons do
                            if IsVector2WithinDrawing(Window.MousePosition, b.Hitbox) then
                                o.Item = b.Text.Text
                                o.Text.Text = o.Item
                                o:Toggle()
                    ...
                    if (o.Type == "Toggle" or o.Type == "ComboBox") and IsVector2WithinDrawing(Window.MousePosition, o.Hitbox) then
                        o:Toggle()
                        break
                    end
                    if o.Type == "Button" and IsVector2WithinDrawing(Window.MousePosition, o.Hitbox) then
                        o.Callback()
                    end
                    if o.Type == "Slider" and IsVector2WithinDrawing(Window.MousePosition, o.Hitbox) then
                        o.BeingDragged = true
                        RunService:BindToRenderStep("DragSlider", 1, function()
                            if o.BeingDragged then
                                local newval = (Window.MousePosition.X - o.Hitbox.Position.X) / o.Hitbox.Size.X
                                newval = math.clamp(newval, 0, 1)
                                local _min = o._min
                                local _max = o._max
                                local v = math.floor(_min + (_max - _min) * newval)
                    ...
                    if o.Type == "ColorPicker" then
                        if IsVector2WithinDrawing(Window.MousePosition, o.Hue) then
                            o.HueDragged = true
                            RunService:BindToRenderStep("DragHue", 1, function()
                                if o.HueDragged then
                                    local percent = (Window.MousePosition.X - o.Hue.Position.X) / o.Hue.Size.X
                                    percent = math.clamp(percent, 0, 1)
                                    o.Picker.Color = Color3.fromHSV(percent, 1, 1)
                                    if o.Items[o.SelectedItem] ~= nil then
                                        local h, s, v = o.Items[o.SelectedItem].Color:ToHSV()
                                        o.Items[o.SelectedItem].ColorBG.Color = Color3.fromHSV(percent, s, v)
                                        o.Items[o.SelectedItem].Color = Color3.fromHSV(percent, s, v)
                                    end
                                else
                        ...
                        if IsVector2WithinDrawing(Window.MousePosition, o.Picker) then
                            o.PickerDragged = true
                            RunService:BindToRenderStep("DragPicker", 1, function()
                                if o.PickerDragged then
                                    local MouseLocation = Window.MousePosition
                                    ...
                                    if o.Items[o.SelectedItem] ~= nil then
                                        local h, s, v = o.Picker.Color:ToHSV()
                                        o.Items[o.SelectedItem].ColorBG.Color = Color3.fromHSV(h, percentx, percenty)
                                        o.Items[o.SelectedItem].Color = Color3.fromHSV(h, percentx, percenty)
                                    end
                                else
                        ...
                        for i2, o2 in next, o.Items do
                            local item_was_selected = false
                            if IsVector2WithinDrawing(Window.MousePosition, o2.BG) then
                                o.SelectedItem = i2 
                    ...
                    if o.Type == "TextBox" and IsVector2WithinDrawing(Window.MousePosition, o.Hitbox) then
                        o.Focused = true
                        o.InputConnection = UIS.InputBegan:Connect(function(_)
    ...
    Connections.OnMouseMove = UIS.InputChanged:Connect(function(_)
        if Window.Mouse and _.UserInputType == Enum.UserInputType.MouseMovement then
            Window.Mouse.Visible = (IsVector2WithinDrawing(Window.MousePosition, Window.MainFrame) or IsVector2WithinDrawing(Window.MousePosition, Window.TitleBar)) and Window.Visible
            Window.Mouse.Position = Window.MousePosition
            if Window.BeingDragged then
                local Delta = (Window.MousePosition - Window.LastMouseClickOnTitle)
                Window.LastMouseClickOnTitle = Window.MousePosition
    ...
    local LastPosition, LastViewportSize = Window.MainOutline.Position, Camera.ViewportSize
    return Window
end
